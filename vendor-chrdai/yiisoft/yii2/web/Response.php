...
class Response extends \yii\base\Response
{
    ...
    /**
     * @var mixed 原始响应数据。当该值不为空时，它将被转换为[[content]]根据[[format]]发送响应。
     */
    public $data;

    /**
     * @var string 响应内容。当[[data]]不为空时，将转换为[[content]],根据[[格式]]发送响应。
     * @see data
     */
    public $content;

    /**
     * @var string 响应格式。这决定了如何将[[data]]转换成[[content]]
     * 当未设置后者时。此属性的值必须是[[formatters]]数组中声明的键之一。
     * 默认情况下，支持以下格式:
     *
     * - [[FORMAT_RAW]]:数据将被视为响应内容，不需要任何转换。不会添加额外的HTTP头。
     * - [[FORMAT_HTML]]:数据将被视为响应内容，没有任何转换。“Content-Type”标头将设置为“text/html”。
     * - [[FORMAT_JSON]]:数据将被转换成JSON格式，以及“Content-Type”,header将被设置为“application/json”。
     * - [[FORMAT_JSONP]]:数据将被转换为JSONP格式，以及“Content-Type”,标题将设置为“text/javascript”。注意，在本例中' $data '必须是一个数组
     *   带有“data”和“callback”元素。前者是指要发送的实际数据，而后者指的是JavaScript回调函数的名称。
     * - [[FORMAT_XML]]:数据将被转换成XML格式。请参考[[XmlResponseFormatter]]
     *
     * 您可以通过配置[[formatters]]自定义格式化过程或支持其他格式。
     */
